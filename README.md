# 👋 Olá, eu sou a Thaís (@0thferr)

💻 Software Engineer | Full Stack Developer | IA Generativa  
🎓 Engenheira de Computação — UNISAL  
📚 Pós-graduanda em IA Generativa e Aplicações com LLMs — PUC Minas

Desenvolvo aplicações web, APIs e soluções inteligentes com foco em escalabilidade, automação, performance e Inteligência Artificial. Tenho experiência prática com backend, frontend, DevOps, integração de APIs e aplicações com IA generativa.

---

# 🔧 Tecnologias & Ferramentas

Python • JavaScript • TypeScript • C • C++ • React • Vue.js • HTML • CSS • Node.js • Express.js • Flask • MySQL • MongoDB • ChromaDB • Docker • Kubernetes • CI/CD • GitHub Actions • GitLab CI • Firebase • Git • Power BI • Linux Ubuntu • OpenCV • YOLO • Machine Learning • IA Generativa • LLMs • RAG • Prompt Engineering • Embeddings

---

# 🚀 Experiência

## Desenvolvimento Full Stack
- Desenvolvimento de aplicações web completas
- Construção de APIs REST
- Integração entre sistemas e serviços
- Implementação de autenticação com Firebase

## Inteligência Artificial
- Projetos com visão computacional
- Reconhecimento facial
- Aplicações com IA Generativa
- Estudos e implementação de sistemas com LLMs e RAG

## DevOps & Performance
- Containerização com Docker
- Pipelines CI/CD
- Deploy em produção
- Otimização de performance com CUDA/GPU

---

# 🌱 Atualmente estudando

- IA Generativa e aplicações com LLMs
- LangChain e sistemas RAG
- Engenharia de Agentes
- AWS Cloud
- Kubernetes
- MLOps e LLMOps
- Aplicações multimodais

---

# 🤝 Interesse em colaborar

- Projetos Full Stack
- IA Generativa
- Machine Learning
- NLP
- Sistemas com LLMs
- DevOps & Cloud
- Visão Computacional

---

# 📫 Contato

📍 Paulínia - SP  
📧 th.ferr05@gmail.com  
💼 LinkedIn  
---

# 🚀 Experiência

## Desenvolvimento Full Stack
- Desenvolvimento de aplicações web completas
- Construção de APIs REST
- Integração entre sistemas e serviços
- Implementação de autenticação com Firebase

## Inteligência Artificial
- Projetos com visão computacional
- Reconhecimento facial
- Aplicações com IA Generativa
- Estudos e implementação de sistemas com LLMs e RAG

## DevOps & Performance
- Containerização com Docker
- Pipelines CI/CD
- Deploy em produção
- Otimização de performance com CUDA/GPU

---

# 🌱 Atualmente estudando

- IA Generativa e aplicações com LLMs
- LangChain e sistemas RAG
- Engenharia de Agentes
- AWS Cloud
- Kubernetes
- MLOps e LLMOps
- Aplicações multimodais

---

# 🤝 Interesse em colaborar

- Projetos Full Stack
- IA Generativa
- Machine Learning
- NLP
- Sistemas com LLMs
- DevOps & Cloud
- Visão Computacional


## 📫 Contato

* 📧 [thais.ferreira07@outlook.com](mailto:thais.ferreira07@outlook.com)
* 💼 [LinkedIn](https://www.linkedin.com/in/tha%C3%ADs-silva-ferreira-b1b6b61a4/)

---

⭐ Curtiu algum projeto? Deixa uma estrela.
